# Ingegneria del Software Avanzata

Questo repository contiene il materiale utilizzato in laboratorio per il corso Ingegneria del Software Avanzata all'Università di Ferrara.

Docente: Damiano Azzolini.

## Risorse

- [Syllabus](https://unife.coursecatalogue.cineca.it/corsi/2025;anno=2025)
- [Aule](https://aule.unife.it/AgendaStudenti/index.php?view=easycourse&_lang=it)
- Google Classrom: ugtkaxp4
- Materiale Didattico: questo repository

Suggerisco di avere un ambiente **Linux** funzionante (durante le lezioni verrà utilizzato WSL, ma una qualsiasi distro Linux va bene).

## Lezioni 2026

| #  | Data | Argomento |
| -- | -- |-------- |
| 01 | 27/02/2026 | Introduzione e metodi agili |
| 02 | 06/03/2026 | Git |
| 03 | 09/03/2026 | TODO |
| 04 | TODO | TODO |
| 05 | TODO | TODO |
| 06 | TODO | TODO |
| 07 | TODO | TODO |
| 08 | TODO | TODO |
| 09 | TODO | TODO |
| 10 | TODO | TODO |
| 11 | TODO | TODO |
| 12 | TODO | TODO |
| 13 | TODO | TODO |


## Come Convertire MD in PDF
Se si preferisce leggere su pdf, suggerisco di installare `pandoc`

`sudo apt install pandoc`

e poi utilizzare il comando

`pandoc --from=gfm --to=pdf <file_da_convertire> -o <file_di_output>.pdf`